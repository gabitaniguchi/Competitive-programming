...
struct state {
    ...
    int min_pos, max_pos;
    // Se o alfabeto for pequeno (ex: 'a'-'z'), use int next[26] para economizar MUITA memória
    map<char, int> next; 
};
...
int32_t main() {
    _
    int q; cin >> q;
    vector<string> padroes(q);
    for(int i=0; i<q; i++) cin >> padroes[i];
    ...
    for(int i=0; i<q; i++){
        ...
        bool possivel = true;
        for(char c : w) {
            if(!sam[estado].next.count(c)) {
                possivel = false;
                break;
            }
            estado = sam[estado].next[c];
        }

        if(!possivel) {
            cout << "-1 -1" << endl;
        } else {
            // A posição de início é (posição de fim - tamanho + 1)
            int first = sam[estado].min_pos - w.size() + 1;
            int last_pos = sam[estado].max_pos - w.size() + 1;
            cout << first << " " << last_pos << endl;
        }
    }
}